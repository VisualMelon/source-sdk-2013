...
void CSkinButton::Paint ( void )
{
	...
	if ( pThisMenu->m_iTeamSelection == TEAM_BRITISH)
	{
		...
		{
			m_pImage = scheme()->GetImage( MouseoverImageBrit, false );
		}
		else
		{
			m_pImage = scheme()->GetImage( ImageBrit, false );
		}
	}
	else if ( pThisMenu->m_iTeamSelection == TEAM_AMERICANS)
	{
		if ( IsSelected() || m_bMouseOver )
		{
			m_pImage = scheme()->GetImage( MouseoverImageAmer, false );
		}
		else
		{
			m_pImage = scheme()->GetImage( ImageAmer, false );
		}
	}
...
void CStatsButton::GetWeaponImage( char *m_szImage )
{
	int wide, tall;
	vgui::IImage *m_pImage = NULL;
	...
	m_pImage = scheme()->GetImage( m_szImage, false );
